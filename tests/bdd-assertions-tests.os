...
Функция ПолучитьСписокТестов(юТест) Экспорт
	...
	ВсеТесты.Добавить("ТестДолжен_Проверить_Содержит_ДляМногострочныхСтрок2");
	ВсеТесты.Добавить("ТестДолжен_Проверить_Содержит_ДляМногострочныхСтрок_НеНайдено");
...
Процедура ТестДолжен_Проверить_Содержит_ДляМногострочныхСтрок2() Экспорт
	Строка = "BDD for OneScript ver.1.13.0
|	Функциональность Просто функционал
|	   Сценарий Просто сценарий
|	ПРЕДУПРЕЖДЕНИЕ   -       выполняется несуществующий шаг
|				Не реализован
|
|
|
...
|
|	ПРЕДУПРЕЖДЕНИЕ   - 1 Сценарий ( 0 Пройден, 1 Не реализован, 0 Сломался, 0 Не выполнялся )
|	ПРЕДУПРЕЖДЕНИЕ   - 1 Шаг ( 0 Пройден, 1 Не реализован, 0 Сломался, 0 Не выполнялся )";

	ЧтоИщем = "Проблемные сценарии:
	|Функциональность Просто функционал
	|	Сценарий Просто сценарий
	|		Шаг выполняется несуществующий шаг - Не реализован";
...
Процедура ТестДолжен_Проверить_Содержит_ДляМногострочныхСтрок_НеНайдено() Экспорт
	Строка = "BDD for OneScript ver.1.13.0
|	Функциональность Просто функционал
|	   Сценарий Просто сценарий
|	ПРЕДУПРЕЖДЕНИЕ   -       выполняется несуществующий шаг
...
|	ПРЕДУПРЕЖДЕНИЕ   - 1 Шаг ( 0 Пройден, 1 Не реализован, 0 Сломался, 0 Не выполнялся )";
	...
	|Функциональность Просто функционал
	|	Сценарий Просто сценарий
	|		Шаг несуществующий шаг - Не реализован";

	Ожидаем.Что(Строка, "Строка").ЭтоНе().Содержит(ЧтоИщем);
КонецПроцедуры
...
Процедура ТестДолжен_Проверить_ЧтоВыбрасываетсяИсключение() Экспорт
	...
	ОжидаемыйТекстИсключения = "Задано неправильное имя атрибута структуры";

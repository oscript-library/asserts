...
Функция РазличияКолонокТаблиц(Знач Таб1, Знач Таб2)
	...
	Для Каждого Различие Из РазличияИменКолонок(Таб1, Таб2, Ложь) Цикл
		ЗаполнитьЗначенияСвойств(Различия.Добавить(), Различие);
	КонецЦикла;

	Для Каждого Различие Из РазличияИменКолонок(Таб2, Таб1, Истина) Цикл
		ЗаполнитьЗначенияСвойств(Различия.Добавить(), Различие);
	КонецЦикла;
...
Функция РазличияИменКолонок(Знач Таб1, Знач Таб2, Знач ПоменятьМестами)
	
	Различия = ПустаяТаблицаРазличийТаблиц();

	Сообщение = ?(ПоменятьМестами, "ОЖИДАЕМОЕ значение", "ПРОВЕРЯЕМОЕ значение");

	Для Кол = 0 По Таб1.Колонки.Количество() - 1 Цикл
		КолонкаТаб1 = Таб1.Колонки[Кол];
		КолонкаТаб2 = ?(ПараметрыСравненияТаблиц.УчитыватьПорядокКолонок,
							Таб2.Колонки[Кол],
							Таб2.Колонки.Найти(КолонкаТаб1.Имя));
		Если КолонкаТаб2 = Неопределено Или Не КолонкаТаб1.Имя = КолонкаТаб2.Имя Тогда
			Ожидание = ?(КолонкаТаб2 = Неопределено, "<ОТСУТСТВУЕТ>", СтрШаблон("колонку ""%1""", КолонкаТаб2.Имя));
			Результат = СтрШаблон("колонку ""%1""", КолонкаТаб1.Имя);
			Различие = РазличиеТаблиц(Сообщение,
										"Колонки",
										?(ПоменятьМестами, Результат, Ожидание),
										?(ПоменятьМестами, Ожидание, Результат));
			ЗаполнитьЗначенияСвойств(Различия.Добавить(), Различие);
		КонецЕсли;
	КонецЦикла;
...
Функция РазличияСтрокТаблиц(Знач ПроверяемоеЗначение, Знач ОжидаемоеЗначение)
	...
	Если Не Таб1.Количество() = Таб2.Количество() Тогда
		Различие = РазличиеТаблиц("ПРОВЕРЯЕМОЕ значение",
									"Количество СТРОК",
									Таб2.Количество(),
									Таб1.Количество());
	...
	Для Стр = 0 По Таб1.Количество() - 1 Цикл
		Для Кол = 0 По Таб1.Колонки.Количество() - 1 Цикл
			...
			Тогда
				...
				Ожидание = ?(КолонкаТаб2 = Неопределено, "<НЕТ КОЛОНКИ>", Таб2[Стр][КолонкаТаб2]);
			Иначе
